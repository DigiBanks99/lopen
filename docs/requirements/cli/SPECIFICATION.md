...
- [ ] [CLI-01] `lopen` (root command) starts the TUI with full workflow; offers session resume if active session exists
- [ ] [CLI-02] `lopen --headless` runs the full workflow autonomously with plain text output to stdout/stderr
- [ ] [CLI-03] `lopen spec` runs the Requirement Gathering phase (step 1) with guided conversation
- [ ] [CLI-04] `lopen plan` runs the Planning phase (steps 2–5); errors if no specification exists for the target module
- [ ] [CLI-05] `lopen build` runs the Building phase (steps 6–7); errors if no specification and plan exist
- [ ] [CLI-06] `lopen auth login` initiates the Copilot SDK device flow
- [ ] [CLI-07] `lopen auth status` reports current authentication state
- [ ] [CLI-08] `lopen auth logout` clears SDK-managed credentials
- [ ] [CLI-09] `lopen session list` lists all sessions (active and completed)
- [ ] [CLI-10] `lopen session show` displays session details with optional `--format` flag
- [ ] [CLI-11] `lopen session resume [id]` resumes a specific session
- [ ] [CLI-12] `lopen session delete <id>` deletes a session
- [ ] [CLI-13] `lopen session prune` removes completed sessions beyond retention limit
- [ ] [CLI-14] `lopen config show` displays resolved configuration with sources
- [ ] [CLI-15] `lopen revert` rolls back to the last task-completion commit and updates session state
- [ ] [CLI-16] `--headless` / `--quiet` / `-q` disables TUI entirely; output is plain text to stdout/stderr
- [ ] [CLI-17] `--prompt <text>` injects user instructions into LLM context
- [ ] [CLI-18] `--prompt` in TUI mode populates the input field for user review before sending
