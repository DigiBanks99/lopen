...
- [ ] [TUI-50] A `TuiOutputRenderer` implementation of `IOutputRenderer` bridges orchestrator output events to the TUI activity panel, replacing the default `HeadlessRenderer` when TUI mode is active
...
- [ ] [TUI-52] TUI `RunAsync` launches the `WorkflowOrchestrator` on a background thread and renders its progress in real time — the TUI is not a passive shell but actively drives and displays the workflow
